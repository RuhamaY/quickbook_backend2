...
export default function VendorInsights() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Extract vendor from search results
      const vendors = vendorsResult.data.QueryResponse?.Vendor || []
      if (vendors.length === 0) {
          toast({
            title: "No vendor found",
            description: `We couldn't find any vendor matching "${query}".`,
      ...
      if (transactionsResult.data) {
        const allPurchases = transactionsResult.data.QueryResponse?.Purchase || []
        ...
        const totalSpend = vendorExpenses.reduce((sum, item) => sum + (item.TotalAmt || 0), 0)
        ...
        const lastDate = sortedMatches.length > 0 ? sortedMatches[0].TxnDate : new Date().toISOString().split("T")[0]
        ...
        setData({
          ...
          summary_text: `Found ${count} transactions for ${vendor.DisplayName}. This vendor accounts for ${formatCurrency(totalSpend)} in total spend, with an average transaction size of ${formatCurrency(avgTransaction)}.`,
  ...
  return (
    <Card className="h-full border-none shadow-md bg-gradient-to-br from-background to-slate-50/50 dark:from-background dark:to-slate-950/50">
      <CardHeader>
        ...
        <CardDescription>Search for any vendor to generate real-time spending analysis.</CardDescription>
      </CardHeader>
        ...
        {data ? (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-blue-50 dark:bg-blue-950/30 p-4 rounded-lg border border-blue-100 dark:border-blue-900/50 shadow-sm">
              <div className="flex gap-3 items-start">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-blue-700 dark:text-blue-300 leading-relaxed">{data.summary_text}</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="p-4 bg-background border rounded-xl shadow-sm hover:shadow-md transition-shadow">
                ...
                <p className="text-2xl font-bold mt-1 text-primary">{formatCurrency(data.metrics.total_spend)}</p>
              </div>
              <div className="p-4 bg-background border rounded-xl shadow-sm hover:shadow-md transition-shadow">
                <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Transactions</p>
                <p className="text-2xl font-bold mt-1">{data.metrics.transaction_count}</p>
              </div>
              <div className="p-4 bg-background border rounded-xl shadow-sm hover:shadow-md transition-shadow">
                <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Avg Transaction</p>
                <p className="text-2xl font-bold mt-1">{formatCurrency(data.metrics.avg_transaction)}</p>
              </div>
              <div className="p-4 bg-background border rounded-xl shadow-sm hover:shadow-md transition-shadow">
                <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Last Seen</p>
                <p className="text-2xl font-bold mt-1">{data.metrics.last_transaction_date}</p>
              </div>
            </div>
            ...
            <div className="h-[200px] w-full mt-4 p-4 bg-background border rounded-xl shadow-sm">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h4 className="text-sm font-semibold text-muted-foreground">Recent Spending Trend</h4>
              </div>
          ...
          !loading && (
            <div className="flex flex-col items-center justify-center h-32 text-center space-y-2 text-muted-foreground border-2 border-dashed rounded-xl bg-slate-50/50 dark:bg-slate-900/50">
              ...
              <p className="text-sm">Enter a vendor name above to see AI-powered insights</p>
            </div>
          )
        )}
      </CardContent>
